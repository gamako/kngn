...
wait_masks() {
  ...
  for i in $(seq 1 2000); do
    ...
    out=$("$KNGN" ctl --port-file "$port_file" 'step 1; digest modular' 2>/dev/null || true)
...
"$KNGN" ctl --port-file "$SOLO_PORT" "$PATTERN_ACTIONS"

before_digest=$(wait_masks "$SOLO_PORT" "$solo_pid")
log "solo before masks: $(printf '%s' "$before_digest" | grep -oE '"patterns":\{[^}]+\}')"
"$KNGN" ctl --port-file "$SOLO_PORT" "snapshot fb $WORKDIR/solo-restored.png"
"$KNGN" ctl --port-file "$SOLO_PORT" "action save_project $BEFORE"
"$KNGN" ctl --port-file "$SOLO_PORT" 'action toggle_step kick 1'
"$KNGN" ctl --port-file "$SOLO_PORT" 'step 1'
"$KNGN" ctl --port-file "$SOLO_PORT" "action load_project $BEFORE"

after_load_digest=$(wait_masks "$SOLO_PORT" "$solo_pid")
log "solo after load masks: $(printf '%s' "$after_load_digest" | grep -oE '"patterns":\{[^}]+\}')"
"$KNGN" ctl --port-file "$SOLO_PORT" "action save_project $AFTER"
...
"$KNGN" ctl --port-file "$HOST_PORT" "$PATTERN_ACTIONS"
...
for i in $(seq 1 2000); do
  ...
  "$KNGN" ctl --port-file "$HOST_PORT" 'step 1' >/dev/null || true
  net=$("$KNGN" ctl --port-file "$CLIENT_PORT" 'step 1; digest netsync' 2>/dev/null || true)
...
for i in $(seq 1 2000); do
  ...
  "$KNGN" ctl --port-file "$HOST_PORT" 'step 1' >/dev/null || true
  client_out=$("$KNGN" ctl --port-file "$CLIENT_PORT" 'step 1; digest modular' 2>/dev/null || true)
...
host_digest=$("$KNGN" ctl --port-file "$HOST_PORT" 'digest modular')
client_digest=$("$KNGN" ctl --port-file "$CLIENT_PORT" 'digest modular')
