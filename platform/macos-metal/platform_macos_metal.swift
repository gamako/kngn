...
class MetalFramebufferView: MTKView, NSTextInputClient {
    ...
    func firstRect(forCharacterRange range: NSRange, actualRange: NSRangePointer?) -> NSRect {
        ...
        if compositionRectSet && compositionRectPixels.width > 0 && compositionRectPixels.height > 0 {
            ...
            let fbW = metalRenderer?.getWidth() ?? 0
            let fbH = metalRenderer?.getHeight() ?? 0
            let sx = fbW > 0 ? bounds.width / CGFloat(fbW) : 1.0
            let sy = fbH > 0 ? bounds.height / CGFloat(fbH) : 1.0
...
// TASK-117: 現在のウィンドウ geometry。位置=frame.origin、サイズ=content（Metal view の fb ではなく NSWindow content）。
...
func platform_set_text_input_document_access(
    ...
    guard let platformWindow = platformWindow else { return }
    let handle = Unmanaged<PlatformWindowHandle>.fromOpaque(platformWindow).takeUnretainedValue()
    handle.metalView.setTextInputDocumentAccess(callbacks: callbacks, userdata: userdata)
}
