...
**File: builtin.CallModifier struct.zig**
```zig
...
**File: not_atomic_cmpxchgStrong.zig**
```zig
...
**File: test_compileLog_builtin.zig**
```zig
...
**File: export_builtin_equivalent_code.zig**
```zig
export fn foo() void {}
```
...
**File: export_any_symbol_name.zig**
```zig
...
1/1 test_intCast_builtin.test.integer cast panic...thread 3100061 panic: integer does not fit in destination type
...
/home/ci/actions-runner/_work/zig-bootstrap/out/zig-local-cache/o/a1024772a598ec6bec7bf5d3fc4d0297/test --seed=0xbbb33479
```
...
**File: test_wasmMemoryGrow_builtin.zig**
```zig
const std = @import("std");
...
**File: test_without_setEvalBranchQuota_builtin.zig**
```zig
test "foo" {
 comptime {
 var i = 0;
 while (i `@setFloatMode(comptime mode: FloatMode) void`
...
- `Optimized` - Floating point operations may do all of the following: Assume the arguments and result are not NaN. Optimizations are required to retain legal behavior over NaNs, but the value of the result is undefined.
...
**File: test_setRuntimeSafety_builtin.zig**
```zig
...
1/1 test_setRuntimeSafety_builtin.test.@setRuntimeSafety...thread 3103195 panic: integer overflow
/home/ci/actions-runner/_work/zig-bootstrap/zig/doc/langref/test_setRuntimeSafety_builtin.zig:11:11: 0x1050078 in test.@setRuntimeSafety (test)
 x += 1;
 ^
/home/ci/actions-runner/_work/zig-bootstrap/out/host/lib/zig/compiler/test_runner.zig:240:25: 0x104640f in mainTerminal (test)
 if (test_fn.func()) |_| {
 ^
/home/ci/actions-runner/_work/zig-bootstrap/out/host/lib/zig/std/start.zig:618:22: 0x104409d in callMain (test)
 root.main();
 ^
/home/ci/actions-runner/_work/zig-bootstrap/out/host/lib/zig/std/start.zig:232:5: 0x1043d7d in _start (test)
 asm volatile (switch (native_arch) {
 ^
error: the following test command crashed:
/home/ci/actions-runner/_work/zig-bootstrap/out/zig-local-cache/o/dfb6236d4c5704abd528772026e355ec/test --seed=0x4bb67001
```
...
**File: test_truncate_builtin.zig**
```zig
const std = @import("std");
const expect = std.testing.expect;
