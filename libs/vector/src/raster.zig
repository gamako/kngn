// Outline rasterizer. Converts a shared Outline into 8bpp coverage.
//
...
/// Maps outline coordinates into the destination pixel grid:
/// `device.x = v.x * sx + dx`, `device.y = v.y * sy + dy`.
/// Source units are the caller's. A Y-up source targeting a Y-down buffer uses sy < 0.
/// Font-unit placement (including the conventional sy < 0 flip) stays in font as `Transform`.
pub const ScaleTranslate = struct {
    sx: f32,
    sy: f32,
    dx: f32,
    dy: f32,

    fn apply(self: ScaleTranslate, v: Vec2f) Vec2f {
...
const identity = ScaleTranslate{ .sx = 1, .sy = 1, .dx = 0, .dy = 0 };
...
// Expected Fnv1a_32 of the 8bpp coverage for these fixtures. A mismatch is a
// behaviour change in the rasterizer.
test "raster: coverage hashes stay bit-identical" {
    const a = testing.allocator;

    {
        ...
        const bm = try rasterize(a, ol, identity, 6, 6);
        defer a.free(bm.data);
        try testing.expectEqual(@as(u32, 0x3A87E026), std.hash.Fnv1a_32.hash(bm.data));
    }

    {
        ...
        const bm = try rasterize(a, ol, identity, 4, 4);
        defer a.free(bm.data);
        try testing.expectEqual(@as(u32, 0x3AAE4711), std.hash.Fnv1a_32.hash(bm.data));
    }

    {
        ...
        const r: f32 = 6;
        try b.moveTo(.{ .x = cx + r, .y = cy });
        try b.quadTo(.{ .x = cx + r, .y = cy + r }, .{ .x = cx, .y = cy + r });
        try b.quadTo(.{ .x = cx - r, .y = cy + r }, .{ .x = cx - r, .y = cy });
        try b.quadTo(.{ .x = cx - r, .y = cy - r }, .{ .x = cx, .y = cy - r });
        try b.quadTo(.{ .x = cx + r, .y = cy - r }, .{ .x = cx + r, .y = cy });
        var ol = try b.finish();
        defer ol.deinit(a);
        const bm = try rasterize(a, ol, identity, 16, 16);
        defer a.free(bm.data);
        try testing.expectEqual(@as(u32, 0x965F1091), std.hash.Fnv1a_32.hash(bm.data));
    }

    {
        ...
        errdefer b.deinit();
        const pts = [_][3]Vec2f{
            .{ .{ .x = 24, .y = 4 }, .{ .x = 32, .y = 4 }, .{ .x = 36, .y = 12 } },
            .{ .{ .x = 40, .y = 20 }, .{ .x = 40, .y = 28 }, .{ .x = 36, .y = 36 } },
            .{ .{ .x = 32, .y = 44 }, .{ .x = 24, .y = 44 }, .{ .x = 16, .y = 44 } },
            .{ .{ .x = 8, .y = 44 }, .{ .x = 4, .y = 36 }, .{ .x = 4, .y = 28 } },
            .{ .{ .x = 4, .y = 20 }, .{ .x = 4, .y = 12 }, .{ .x = 8, .y = 4 } },
            .{ .{ .x = 12, .y = 2 }, .{ .x = 16, .y = 2 }, .{ .x = 20, .y = 2 } },
            .{ .{ .x = 22, .y = 2 }, .{ .x = 23, .y = 3 }, .{ .x = 24, .y = 4 } },
            .{ .{ .x = 24, .y = 4 }, .{ .x = 24, .y = 4 }, .{ .x = 24, .y = 4 } },
        };
        try b.moveTo(.{ .x = 20, .y = 2 });
        for (pts) |p| try b.cubicTo(p[0], p[1], p[2]);
        var ol = try b.finish();
        defer ol.deinit(a);
        const bm = try rasterize(a, ol, identity, 48, 48);
        defer a.free(bm.data);
        try testing.expectEqual(@as(u32, 0x737342D0), std.hash.Fnv1a_32.hash(bm.data));
    }
}
