// Shared path representation. A Contour is a closed path (implicitly closes
// from the last segment end back to start). Coordinates are f32 in the
// caller's units; the rasterizer does not interpret them.
//
// This type lives here rather than in font: the coverage fill is a generic
// path operation, and font is one producer (emitting font-unit coordinates).
