//! libs/gfx umbrella（TASK-111.2 / TASK-111.3）。
//! sprite / fixed_timestep / fps_counter / keyboard / atlas / animation を再エクスポートし、
//! kit.gfx の公開面とする。
//!
//! atlas / animation は同一 module 内の相対 import（`@import("atlas.zig")` 等）。
//! こうすると `zig test` が gfx root 経由で両ファイルの test を収集し、専用 runner を
//! 増やさずに test-gfx へ接続できる（TASK-111.3 Claude 設計レビュー付記）。
