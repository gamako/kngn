...
pub const Hvar = struct {
    ...
    pub fn parse(table: []const u8, axis_count: u16) Error!Hvar {
        ...
        // IVS format 検証
        try validateIvs(table, ivs_off, axis_count);
    ...
    pub fn advanceDelta(self: *const Hvar, gid: u16, norm: []const f32) Error!f32 {
        ...
        return try itemDelta(self.data, self.ivs_off, outer_inner.outer, outer_inner.inner, norm, self.axis_count);
    }
...
fn validateIvs(table: []const u8, ivs_off: usize, axis_count: u16) Error!void {
    const r = Reader{ .data = table };
...
fn itemDelta(
    ...
    if (ivd_rel == 0) return 0; // NULL offset
    ...
    while (col < region_index_count) : (col += 1) {
        ...
        // region scalar（VariationRegion = axisCount × (start, peak, end) F2DOT14）
        ...
        // ItemVariationStore の region は常に start/peak/end を持つ（gvar の intermediate と同型）
        const scalar = gvar_mod.tupleScalar(norm[0..ac], peak_buf[0..ac], true, start_buf[0..ac], end_buf[0..ac]);
        total += delta * scalar;
    }
    return total;
}
