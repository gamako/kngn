//! straight-alpha src-over ブレンド（TASK-21.11）。0xAABBGGRR、非 premultiplied。
//!
...
/// src OVER dst（**引数順: dst が先・src が後**）。straight-alpha src-over, 0xAABBGGRR。
pub fn srcOver(dst: u32, src: u32) u32 {
    ...
    const r = (ch(src, 0) * sa * 255 + ch(dst, 0) * da * inv + oa255 / 2) / oa255;
    const g = (ch(src, 8) * sa * 255 + ch(dst, 8) * da * inv + oa255 / 2) / oa255;
    const b = (ch(src, 16) * sa * 255 + ch(dst, 16) * da * inv + oa255 / 2) / oa255;
    return (out_a << 24) | (b << 16) | (g << 8) | r;
}
...
test "srcOver: 透明 dst へ半透明 src（out_a 計算）" {
    // dst 完全透明 (a=0)、src a=128 の赤(R=255)
    const out = srcOver(0x00000000, 0x80_00_00_FF); // 0xAABBGGRR: a=80,b=00,g=00,r=FF
    const oa = (out >> 24) & 0xFF;
    try std.testing.expectEqual(@as(u32, 128), oa); // out_a = sa + 0 = 128
    // R チャネル: (255*128*255 + 0)/( 128*255 ) = 255
    try std.testing.expectEqual(@as(u32, 255), out & 0xFF);
}

test "srcOver: 不透明赤の上に半透明青（中間ブレンド）" {
    const dst: u32 = 0xFF0000FF; // 不透明赤（r=FF）
    const src: u32 = 0x80FF0000; // a=80, b=FF（青）, g=0, r=0
    const out = srcOver(dst, src);
    try std.testing.expectEqual(@as(u32, 0xFF), (out >> 24) & 0xFF); // out_a=255（da=255）
    // r = (0*128*255 + 255*255*127 + .. )/(128*255+255*127) ... 近似: ~127
    const r = out & 0xFF;
    const b = (out >> 16) & 0xFF;
