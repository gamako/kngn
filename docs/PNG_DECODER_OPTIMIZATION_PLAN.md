...
4. **ステップ 4: Phase 2.1 - ローカル変数最適化** ✅ **完了**
   - [x] flate.zig でローカル変数最適化 ✅
   - [x] inline キーワードの検証（却下） ✅
   - [x] テスト実行（全29テスト通過） ✅
   - [x] ベンチマーク実行し改善率を記録 ✅
   - **結果:** 1920x1080 RGBA: 19,105 → 19,010 μs (0.5% 改善)
   - **現状:** Phase 0 から累積で **31.1% 高速化**、74.9% メモリ削減

5. **ステップ 5: 次のフェーズに向けた検討** 🚀 **次の優先タスク**
...
### ステップ 4: Phase 2.1 - ローカル変数最適化（inline化は却下） ✅ **完了**
- [x] flate.zig で bytes_per_pixel と bytes_per_scanline をローカル変数化 ✅
- [x] テスト実行して動作確認（29/29テスト通過） ✅
- [x] ベンチマーク実行（Stage 1: 0.5-1.4% 改善） ✅
- [x] filter.zig の関数に inline キーワード追加して検証 ✅
- [x] ベンチマーク実行（Stage 2: 0.6% 悪化を確認） ✅
- [x] inline を削除し、ローカル変数最適化のみ採用 ✅
- [x] **最終ベンチマーク実行し改善率を記録** ✅
  - **結果:** 1920x1080 RGBA: 19,010 μs (0.5% 改善)
  - **学び:** inline は測定せずに追加すべきではない
...
#### Phase 2.1: ローカル変数最適化（inline化は却下） ✅ **完了**
- ✅ すべてのテストが通る（29/29テスト）
- ✅ ローカル変数最適化: 0.5-1.4% 向上（採用）
  - flate.zig の applyFilterInPlace で bytes_per_pixel と bytes_per_scanline をキャッシュ
  - L1/L2 キャッシュ効率改善
- ❌ inline キーワード追加: 0.6% 悪化（却下）
  - filter.zig の Direct 関数に inline を追加したがパフォーマンスが悪化
  - Zig コンパイラの自動最適化の方が優れている
- **結果:** 1920x1080 RGBA: 19,010 μs (Phase 1.3 の 19,105 μs から 0.5% 改善)
- **累積改善率:** Phase 0 から 31.1% 高速化
