...
3. **ステップ 3: 並行で Phase 1.3 の設計を進める** 🚀 **次の優先タスク**
   - [ ] API デザイン（DeflateReader）の検討
   - [ ] テストケースの準備
   - [ ] 実装計画の詳細化
   - **優先度:** 高 - Phase 1.2 の結果から、Filter type 0 最適化はエンドツーエンド性能への寄与が限定的であり、より大きなボトルネック（バッファ管理）の解決が必要

4. **ステップ 4: Phase 1.3 - ストリーミング化を実装**
   - [ ] flate.zig にストリーミング API を追加
   - [ ] filter.applyFilters を行単位パイプラインに変更
   - [ ] lib.zig のバッファ管理を見直し
   - (前のステップで設計が完了すれば実装が円滑)
