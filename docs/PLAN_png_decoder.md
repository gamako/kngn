...
**Phase 1（必須）: RGBA (Color Type = 6, 8bit)**
- 透明度付きカラー画像
- フォントアトラス、スプライト用として最も一般的
- テスト: 1x1 赤色、8x8 チェッカーボード

**Phase 2: RGB (Color Type = 2, 8bit)**
- 不透明なカラー画像
- RGBAから透明度処理を削除するだけで実装可能
- テスト: グラデーション画像

**Phase 3: Grayscale (Color Type = 0, 8bit)**
- 白黒グレースケール
- モノクロフォント、シンプルなテクスチャ用
- テスト: グレースケール画像
...
### Phase 1: RGBA (8bit) + フィルター (None, Sub, Up)

**テストケース:**
- `1x1_red.png`: 1x1の赤いピクセル（最小テスト）
- `8x8_checkerboard.png`: 8x8 のチェッカーボード（フィルタリング確認）
- `16x16_gradient.png`: グラデーション（データ量確認）
...
**完了条件:**
- テスト全合格
- 実装例（examples/03_sprite_display）で正しく表示される

### Phase 2: RGB (8bit) + フィルター (Average, Paeth)

**テストケース:**
- `16x16_rgb.png`: RGBカラー画像
- `32x32_red_rgb.png`: 不透明な赤

**実装:**
- RGB→RGBA変換（アルファを255で埋める）
- Average フィルタ実装
- Paeth フィルタ実装

### Phase 3: Grayscale (8bit)

**テストケース:**
- `8x8_grayscale.png`: グレースケール
- `16x16_grayscale.png`: より複雑なパターン

**実装:**
- Grayscale→RGBA変換（同じ値をRGBに、アルファ255）
- 色形式判定ロジック
...
**機能検証:**
- ✅ 1x1 赤色画像が正しくデコードされる
- ✅ 8x8 チェッカーボード画像が正しくデコードされる
- ✅ 16x16 グラデーション画像が正しくデコードされる
- ✅ examples/03_sprite_display で正しく表示される
